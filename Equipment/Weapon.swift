...
class Weapon: Equipment {
    ...
    init(version: String, equipmentBag: EquipmentBag, showButton: MSButtonNode){
        ...
        switch version {
        case "1":
            texture = SKTexture(imageNamed: "weapon_1")
            ability = Abiltiy(attackNumber: 9, defenseNumber: 10, healthNumber: 23)
            //self.name = "shoes_1"
        case "2":
            texture = SKTexture(imageNamed: "weapon_2")
        ...
        super.init(texture: texture, type: .weapon, ability: ability, equipmentBag: equipmentBag, showButton: showButton)
        self.name = "weapon"
    }
